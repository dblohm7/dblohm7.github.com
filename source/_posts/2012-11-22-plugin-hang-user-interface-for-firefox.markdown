...
Even though the patch hasn't landed yet, I would like to take a 
moment to explain the feature and describe some noteworthy 
...
its windows always appear in front of the hung Firefox window. I decided 
that I would have Firefox send its top-level `HWND` over to the child 
process that the Hang UI could specify that handle as the owner of its 
top-level windows. If you've seen [Raymond Chen's](http://blogs.msdn.com/b/oldnewthing) 
...
The problem is that the Hang UI windows and the Firefox windows are 
